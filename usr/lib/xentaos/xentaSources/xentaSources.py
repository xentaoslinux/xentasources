                ...
                for element in elements:
                   for protocol in ['http://', 'ftp://', 'https://']:
                        if element.startswith(protocol):
                            ...
                            break
                name = name.replace("xentaos", "Xenta OS")
...
class ComponentToggleCheckBox(gtk.CheckButton):
    ...
    def _on_toggled(self, widget):
        ...
        if widget.get_active() and os.path.exists("/etc/xentaos/info"):
            if self.component.name == "queen":
                if self.application.show_confirmation_dialog(self.application._main_window, _("Xenta OS uses Romeo to publish packages which are not tested. Once these packages are tested, they are then moved to the official repositories. Unless you are participating in beta-testing, you should not enable this repository. Are you sure you want to enable Queen?"), yes_no=True):
...
class MirrorSelectionDialog(object):
    ...
    def __init__(self, application, ui_builder):
        ...
        with open('/usr/lib/xentaos/xentaSources/countries.json') as data_file:
    ...
    def run(self, mirrors, config, is_base):
        ...
        # Try to find the age of the xenta archive
...
class Application(object):
    def __init__(self):
        ...
        glade_file = "/usr/lib/xentaos/xentaSources/xentaSources.glade"
        ...
        self._main_window.set_icon_name("xentasources")
        ...
        config_parser.read("/usr/share/xentasources/%s/xentasources.conf" % self.lsb_codename)
        self.config = {}
        ...
        self.builder.get_object("image_icon").set_from_file("/usr/share/xentasources/%s/icon.png" % self.lsb_codename)
        ...
        if (len(self.optional_components) > 0):
            if os.path.exists("/etc/xentaos/info"):
                # This is xenta, we want to warn people about Romeo
                warning_label = gtk.Label()
                warning_label.set_alignment(0, 0.5)
                warning_label.set_markup("<span font_style='oblique' font_stretch='ultracondensed'>%s</span>" % _("Warning: Unstable packages can introduce regressions and negatively impact your system. Please do not enable these options in Xenta OS unless it was suggested by the development team."))
    ...
    def remove_foreign(self, widget):
        os.system("/usr/lib/xentaos/xentaSources/foreign_packages.py remove %s" % self._main_window.window.xid)

    def downgrade_foreign(self, widget):
        os.system("/usr/lib/xentaos/xentaSources/foreign_packages.py downgrade %s" % self._main_window.window.xid)

    def fix_purge(self, widget):
        os.system("aptitude purge ~c -y")
        image = gtk.Image()
        image.set_from_file("/usr/lib/xentaos/xentaSources/maintenance.png")
    ...
    def fetch_key(self, widget):
        image = gtk.Image()
        image.set_from_file("/usr/lib/xentaos/xentaSources/keyring.png")
    ...
    def add_ppa(self, widget):
        image = gtk.Image()
        image.set_from_file("/usr/lib/xentaos/xentaSources/ppa.png")
        start_line = ""
                ...
                if repository.line.startswith("deb http://ppa.launchpad.net"):
                    ...
                    if line.endswith("/ubuntu"):
                        ...
                        if os.path.exists(ppa_file):
                            os.system("/usr/lib/xentaos/xentaSources/ppa_browser.py %s %s %s %s" % (self.config["general"]["base_codename"], ppa_owner, ppa_name, self._main_window.window.xid))
                        else:
    ...
    def add_repository(self, widget):
        image = gtk.Image()
        image.set_from_file("/usr/lib/xentaos/xentaSources/3rd.png")
        start_line = ""
    ...
    def apply_official_sources(self, widget=None):
        ...
        template = open('/usr/share/xentasources/%s/official-package-repositories.list' % self.lsb_codename, 'r').read()
        ...
        if (self.builder.get_object("source_code_cb").get_active()):
            template = open('/usr/share/xentasources/%s/official-source-repositories.list' % self.lsb_codename, 'r').read()
            template = template.replace("$codename", self.config["general"]["codename"])
            template = template.replace("$basecodename", self.config["general"]["base_codename"])
            template = template.replace("$optionalcomponents", ' '.join(selected_components))
            template = template.replace("$mirror", self.selected_mirror)
    ...
    def update_flags(self):
        xenta_flag_path = FLAG_PATH % '_generic'
        ...
        pixbuf = gtk.gdk.pixbuf_new_from_file_at_size(xenta_flag_path, -1, FLAG_SIZE)
...
if __name__ == "__main__":
    if os.getuid() != 0:
        ...
        if len(args) > 1 and (args[0] == "add-apt-repository"):
            ...
            config_parser = ConfigParser.RawConfigParser()
            config_parser.read("/usr/share/xentasources/%s/xentasources.conf" % lsb_codename)
